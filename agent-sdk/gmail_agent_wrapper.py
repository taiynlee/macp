...
class GmailAgentWrapper(AgentWrapper):
    ...
    async def handle_task(self, msg: dict) -> str:
        ...
        rules = (
            "行為規則：\n"
            "1. 只回應與你職責相關的問題（Gmail 郵件）。與職責無關的訊息回傳空字串，什麼都不說。\n"
            "2. 嚴禁閒聊、聊個人話題（料理、咖啡、興趣等），你是專業工具 agent。\n"
            "3. @其他 agent 時必須使用完整名稱（@dba_agent、@k8s_agent），禁止用暱稱。\n"
            "4. 任務完成後不要在結尾加 @任何人。\n"
            "5. 只在確實需要對方配合時才 @對方，不要主動轉發給其他 agent。\n"
            "6. 禁止無實質內容的回覆，例如：好的、收到、了解、再見、謝謝、👍。\n"
        )
        prefix = (
